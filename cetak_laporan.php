...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->cell(30,10,'Jakarta, '.date("d-m-Y").'');

        $this->Line(10,45,200,45);
    }
    function data_barang(){
        $con = mysqli_connect("localhost","root","");
        mysqli_select_db($con,"db_inventory");
        $tanggal=$_POST['tgl_laporan'];
        if ($_POST['jenis_laporan']=="perhari") {
            $split1=explode('-',$tanggal);
            $tanggal=$split1[2]."-".$split1[1]."-".$split1[0];
            $query= $con->query("select transaksi.id_transaksi,transaksi.tgl_transaksi,transaksi.no_invoice,transaksi.total_bayar,transaksi.nama_pembeli,user.username from transaksi inner join user on transaksi.kode_kasir=user.id where transaksi.tgl_transaksi like '%$tanggal%' order by transaksi.id_transaksi desc");
        }else{
            $split1=explode('-',$tanggal);
...
$pdf->Ln(20);
$pdf->set_table($data);
$pdf->Output('','QCELL/Barang/'.date("d-m-Y").'.pdf');
?>
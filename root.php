...
class penjualan
{
	...
	{
		$this->con=new mysqli("localhost","root","","db_inventory");
	}
	function __destruct()
	{
		$this->con->close();
	...
	function login($username,$password){
		...
		if (isset($error)) {
			echo "<div class='red'><i class='fa fa-warning'></i> Maaf ".implode(' dan ', $error)." anda kosong.</div>";
		}else{
		$password=sha1($password);
		$query=$this->con->query("select * from user where username='$username' and password='$password' and status='1'");
		...
		if ($query->num_rows > 0) 
		{
			echo "<div class='green'><i class='fa fa-check'></i> Login Berhasil, silahkan tunggu beberapa saat.</div>";
				...
				echo "<div class='red'><i class='fa fa-warning'></i> Maaf username atau password anda salah.</div>";
			}
		}
	}
